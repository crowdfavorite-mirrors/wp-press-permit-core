jQuery(document).ready(function($){$(".pp_group-profile div.pp-group_members, .pp_net_group-profile div.pp-group_members").show();$("#pp-add-permissions").show();$("#pp_current_roles").show();$("a.pp-show-groups").click(function(){$('#userprofile_groupsdiv_pp').show();return false;});$(".pp-member-type > a").click(function(){$(".pp-member-type > a").parent().removeClass('agp-selected_agent').addClass('agp-unselected_agent');$(this).parent().removeClass('agp-unselected_agent').addClass('agp-selected_agent');$('div.pp-member-type').hide();$('.'+$(this).attr('class')).show();return false;});var all_roledata=[];var eid=-1;$(".pp-add-permissions > a").click(function(){$(".pp-add-permissions > a").parent().removeClass('agp-selected_agent').addClass('agp-unselected_agent');$(this).parent().removeClass('agp-unselected_agent').addClass('agp-selected_agent');$('div.pp-add-permissions').hide();$('.'+$(this).attr('class')).show();});$("#pp_save_roles input.button-primary").click(function(){$('input[name="member_csv"]').val($("input#member_csv").val());$('input[name="group_name"]').val($("input#group_name").val());$('input[name="description"]').val($("input#description").val());$("#pp_new_submission_msg").html(ppCred.submissionMsg);$("#pp_new_submission_msg").show();});$('#agent-profile #submit').click(function(e){$('#pp_review_roles').hide();$('#pp_add_role').remove();});$(document).on('click',"#pp_tbl_role_selections .pp_clear",function(e){var eid=$(this).closest('tr').find('input[name="pp_eid[]"]').val();if(typeof all_roledata[eid]!='undefined'){delete all_roledata[eid];}
